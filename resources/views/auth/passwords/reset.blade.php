...
@section('auth_subtitle', 'Create a new password')
...
<form method="POST" action="{{ route('password.update') }}" class="space-y-6">
    ...
    <div class="space-y-2">
        ...
        <div class="relative">
            ...
            <input id="email" type="email" class="w-full pl-10 py-2 border border-input bg-background rounded-md focus:ring-2 focus:ring-primary/50 focus:outline-none @error('email') border-destructive @enderror" name="email" value="{{ $email ?? old('email') }}" required autocomplete="email" autofocus placeholder="you@example.com" readonly>
        </div>
        @error('email')
